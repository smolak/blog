...
And where are the tags and ttl I mentioned? Bare with me, I'll get to that.

**stash-it** is built around the concept of hooking into the lifecycle of available methods. This allows you to modify the
...
<aside className='py-0.5 px-5 bg-gray-50 rounded'>
    What is this `extra`? It's an _extra_ data you can set on the item. E.g. to store tags for the item, or ttl data.
    (For more information about the interfaces and types, please head out to the [docs](https://jsr.io/@stash-it) and check
    new posts that will cover the topic)
</aside>

Each method, when called, goes through a series of steps. These steps are:

1. **Before hook** - this is the first step. It is called before the actual method on the adapter is executed.
2. **Adapter method call** - step when adapter method (set, get, ... ) is called.
3. **After hook** - this is the last step. It is called after the method on the adapter is executed.
4. **Data is returned** - the result of after hook call.

Let's examine `setItem` method:

```ts
// This is a simplified version of the source-code of this method, to explain the flow.
...
Plugins are the way to extend the functionality of **stash-it**. Right now, you can only create handlers (in plugins) that hook into the
