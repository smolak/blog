...
1. ```ts
   ...
   Method is called. It expects `key`, `value` and optional `extra` arguments.
...
1. ```ts
   ...
   Connection to storage is established. Depending on where you will be storing the data, adapter might do
...
1. Each method has `before...` hook. `setItem`, at this point, is executing any registered hook handlers
   for the `beforeSetItem` hook. All of the methods' `before...` hook handlers get passed the arguments
   passed to that method. Additionaly, stash-it injects a reference to the adapter in use, as for some cases,
   ...
   What that also means is that each handler must return the same set of data, for it to be able to be
   passed to another handler. But not the adapter. Each handler call gets it injected. You only pass around
   the values used for each hook handler. For example:
...
1. Building the key. This method takes the `key` value returned from `beforeSetItem`. This is important,
