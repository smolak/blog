...
1. `setItem` method is called.
   ...
   passed to the method in question. See point #3.
...
1. The `beforeSetItem` hook gets called (thus executing any registered hook handlers for this hook).
   ...
   for the `beforeSetItem` hook. All of the handlers get passed the arguments
   passed to that method. Well, the first handler gets exactly the same values as those passed to the
   `setItem` method, and any subsequent handlers get values returned by the previously called handlers.
   ...
   Let's put this together. Let's call `setItem` with `some_key` and `some_value`. That means, first registered handler
   for this hook will be called with `{ key: 'some_key', value: 'some_value', extra: {}, adapter }` (where `adapter`'s value
   is a reference to the instance of adapter currently in use).

   As mentioned, each handler must return the same set of data (data structure), so that if there are more
   handlers registered, they can use that data. Meaning: `{ key, value, extra }`.
...
1. Building the key.
   ...
   Let's combine now prefixing and checking if an item is not being overwritten. If that were the case,
   you should do the checking for the existence of an item here, as prefixing changes the key, therefore
   that is the key you're looking the item for.

   ```ts
   // first handler that gets called
   async prefixKeyHandler({ key }) {
     const prefixedKey = `some_prefix_${key}`;

     return { key: prefixedKey }
   }

   // second handler that gets called
   async readOnlyHandler({ key }) {
     // now, the `key` is prefixed and that value is used here
     const itemExists = await adapter.hasItem(key);
...
1. Setting the item using the adapter.
   ...
   At this point the values to store the item are passed to the adapter, to be stored (in whatever
   storage you're using, be it memory, redis or anything else).

   Values used come from:

   - result of calling the `beforeSetItem` hook handlers (hence `beforeData.value` and `beforeData.extra` values),
   - result of calling the `buildKey` hook handlers (hence `builtKey` value).
...
1. The `afterSetItem` hook gets called (thus executing any registered hook handlers for this hook).
   ...
   This is a similar step to the one with `beforeSetItem`. This time handlers (the first one registered),
   are called with:

   - `...beforeSetData` - that means, whatever the result of calling `beforeSetItem` hook handlers is,
   - `key` uses the result of calling `buildKey` hook handlers,
   - `item` uses the value returned by the adapter, which is the item that is stored

   In other words, any last values that went through the hook handlers + result of calling the adapter's method
   are used.

   And again, first hook handler gets those values. If there are more registered, they get the values from
   the return of the prevoius one called. Exactly the same behaviour as with prevoius hook handlers calls.
...
1. Finally, the item that got created, and which value got passed through `afterSetItem` hook handler is returned.
...
## `buildKey` method and hook for it

This method and the hook handlers for it are behaving differently.

As there is no "before" and "after" (for the key building), as adapter is not in use to build the key, this method
has only one hook, the `buildKey` one. It was already mentioned and described above. I wanted only to mention
why it differes from the rest.

## Remaining methods and their hooks

All the remaining methods, meaning: `getItem`, `hasItem`, `removeItem`, `setExtra` and `getExtra` have the same
...
For the `after...`, hook handlers are passed result of getting the item (see types for it), or checking if item exists, or deletion
result or getting or setting an extra.

And all of them have the adapter injected as well.

Since this is almost the same as it is for the `setItem` method, I won't get through all of them in detail.
