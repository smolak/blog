...
date: '2024-11-28'
tags: ['docker', 'how to', 'MySQL', 'testing']
draft: false
summary: 'Can you make dockerized MySQL be ready for interacting with it? Yes, you can. Here's how.'
---

While working on MySQL adapter for stash-it, I stumbled upon a problem of having a MySQL instance not being ready
...
First of all, I wanted to have MySQL run inside a Docker. For that I prepared the docker-compose.yaml file:
...
and the DB is up and running. So far, so good.
...
1. `docker compose up -d` starts the MySQL container in detached mode
...
I got a nasty error when my code wanted to connect to the DB. What was going on? Simple - the DB was not ready yet to be interacted with.
...
I added the explanations and some echo statements to visualize the progress.
...
But it turned out not to be deterministic. I had to come up with a better check in the `if` clause.
...
and finally updated the `docker-compose.yaml` file, adding the container name:
...
and executes command `mysql -u "$MYSQL_USER" -p"$MYSQL_ROOT_PASSWORD" -e "SELECT 1`.
...
But I noticed this error (expected), which was a bit noisy and not informative.
...
A nice, and clean, and a flexible way, is to extract those to the environment variables, as if you decide,
or are forced, to use different values, you will be able to change them in one place.
