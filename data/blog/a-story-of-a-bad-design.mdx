...
So we had to pass it some information about that environment. We thought `isProduction` is fine, for now.

```typescript jsx
<Component isProduction />
```

Since logging was an internal part, not something someone should control from the outside, we thought it's fine to have it
...
One of the proposals was to keep the separate properties, but make them mutually exclusive. So, if `isStaging` is set,
`isProduction` would be ignored. And vice versa. And there would be some strategy to handle this.

But the more I thought about it, the more I realized that this is a bad idea. It's a bad design.
...
The solution was to have a single property, `environment`, that would accept a string. And then the component would
decide what to do based on that string. The `environment` property could decide what to log, how to log, and when to log.
And could control more things in the future or ones needed to be implemented right now.
...
There is one more thing to consider (well, there are probably more, but this is not a ultimate guide to good component
API design).
...
only on production and staging, we can just ignore anything that is not `production`, nor `staging`. Simple as that.
...
On the other hand, don't do too abstract APIs. Don't overengineer. Don't make it too complex. Don't make it too simple.
